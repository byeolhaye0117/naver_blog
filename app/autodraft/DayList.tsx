            ...
            {upcoming.map((d, i) => (
              <li key={`${d.date}|${d.topic}|${i}`} className="bd rounded-xl border border-dashed px-3.5 py-3">
                ...
                */}
                ...
                <p className="muted mt-0.5 text-[11px] leading-relaxed">
                  {/*
                    2026-08-28: 자동 초안은 지역 키워드를 쓰지 않는다 (회원: "아예 이 칸을
                    없앨래"). 옛 기록에는 값이 남아 있으므로 있을 때만 말한다.
                  */}
                  {d.keyword
                    ? '제목은 주제(정보성 검색어)로 엽니다. 옆의 지역 키워드는 예전 설정으로 잡힌 것입니다.'
                    : '이 주제가 그대로 제목을 엽니다 — 정보글의 메인 키워드입니다.'}
                </p>
                <div className="mt-2 flex">
                  <button
                    type="button"
