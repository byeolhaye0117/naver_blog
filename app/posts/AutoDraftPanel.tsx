        ...
        <div className="mt-3.5 space-y-5">
          ...
          */}
          ...
          <section>
            ...
            */}
            {/*
              **하루 편수까지 같이 봐야 한다** (2026-08-31 회원 지적: "나는 31일날 자동작성
              설정한적이 없는데 자동설정 되어 있어").

              회원의 실제 설정이 **주제 3개 · 하루 3편**이었다. 이러면 하루에 목록을 한
              바퀴 다 돌아서 **날마다 똑같은 세 주제**가 나온다 — 실제로 08/30 과 08/31 이
              「단기간다이어트 · 여자근육량늘리기 · 남자 가슴운동 순서」로 같았다.
              그런데 예전 경고는 「하나만 담았을 때」만 떴다. 하루 편수를 안 보고 있었다.
            */}
            {(plan.topics ?? []).length > 0 && (plan.topics ?? []).length <= perDayOf(plan) && (
              <p className="mb-2 rounded-xl border border-amber-500/40 bg-amber-500/8 px-3 py-2 text-[11.5px] leading-relaxed font-semibold text-amber-800 dark:text-amber-200">
                담은 주제가 <b>{(plan.topics ?? []).length}개</b>인데 하루 <b>{perDayOf(plan)}편</b>을 쓰십니다 — 하루에
                목록을 한 바퀴 다 돌아서 <b>날마다 똑같은 주제</b>가 나옵니다. 같은 주제로 며칠씩 쓰면 네이버가 유사
                문서로 볼 수 있습니다. 주제를 더 담으시거나(하루 편수의 서너 배쯤), 하루 편수를 줄이시거나, 담은 것을
                모두 빼서 기본 주제 {INFO_TOPICS.length}개로 돌리세요.
              </p>
            )}
            {/* 넉넉해도 며칠 만에 한 바퀴 도는지는 알려준다 — 그게 다음에 겹치는 날이다 */}
            {(plan.topics ?? []).length > perDayOf(plan) && (
              <p className="muted mb-2 text-[11px] leading-relaxed">
                담은 주제 {(plan.topics ?? []).length}개 ÷ 하루 {perDayOf(plan)}편 ={' '}
                <b>약 {Math.floor((plan.topics ?? []).length / perDayOf(plan))}일</b>마다 한 바퀴를 돕니다. 그 뒤로는
                같은 주제가 다시 나옵니다.
              </p>
            )}
          ...
          */}
          <section>
            ...
            {/*
              **여기서 정한 날에만 쓴다고 읽힌다** (2026-08-31 회원 지적: "나는 31일날
              자동작성 설정한적이 없는데 자동설정 되어 있어 왜그런거야").

              예전 문구는 「채우지 않은 날은 잡히지 않는다 — 그 날이 되면 앱이 그날그날 골라
              쓴다」였다. 앞 반절이 「그 날은 안 쓴다」로 읽힌다. 실제로는 **켜 두면
