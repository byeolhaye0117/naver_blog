...
export default function Editor({
  ...
  async function callWrite(extra: { draft?: Draft; issues?: string[] }) {
    ...
    try {
      ...
      /*
       * **「Failed to fetch」를 그대로 보여주지 않는다** (2026-08-28 회원 지적: "글쓰기에서
       * 오류나" — 화면에 그 영어 한 줄만 떠 있었다).
       *
       * 이건 서버가 낸 오류가 아니라 **브라우저가 연결을 놓은 것**이다. 서버는 멀쩡했다:
       * 같은 요청을 프로덕션에 직접 넣어 보니 56초 만에 200 으로 글이 나왔다.
       *
       * 휴대폰에서 자주 난다 — 글 한 편에 1~2분이 걸리는데 그 사이 화면이 꺼지거나 다른
       * 앱으로 넘어가면 브라우저가 요청을 끊는다. 회원이 할 일이 「다시 누르기」와 「화면 켜
       * 두기」인데, 영어 한 줄로는 그걸 알 수 없다.
       *
       * **끊겨도 안 잃는 길**도 함께 알려준다 — 자동 작성의 「지금 한 편 쓰기」는 서버에서만
       * 돌아 초안까지 저장한다.
       */
      const dropped = e instanceof TypeError || (e instanceof Error && /Failed to fetch|NetworkError|Load failed/i.test(e.message))
      setAiMsg(
        aborted
          ? '4분을 기다렸는데 답이 오지 않아 끊었습니다. 「골격만 넣기」로 직접 쓰거나, 잠시 뒤 다시 시도해 주세요.'
          : dropped
            ? '연결이 끊겼습니다 — 서버 오류가 아니라 글을 쓰는 1~2분 사이에 브라우저가 요청을 놓은 것입니다. 휴대폰에서 화면이 꺼지거나 다른 앱으로 넘어가면 자주 그렇습니다. **화면을 켜 둔 채로 다시 눌러 주세요.** 끊겨도 잃지 않으려면 「자동 작성」 탭의 「지금 한 편 쓰기」를 쓰세요 — 그건 서버에서만 돌아서 끊겨도 초안이 저장됩니다.'
            : e instanceof Error
              ? e.message
              : '글 생성 중 오류가 발생했습니다.'
      )
                  ...
                  */}
                  ...
                  {type !== 'info' && (
                    <Field
                      ...
                      hint={
                        /*
                         * **「이런 혜택이 있었어요」를 지웠다** (2026-09-02). 회원 지적:
                         * "후기글에 이벤트 정보를 넣어놨는데 인식하지 못하고 멋대로 작성해."
                         *
                         * 그 문장이 지시문·서식·이 도움말 세 군데에 똑같이 적혀 있었고, 나온
                         * 글도 딱 그 꼴이었다 — 금액 하나만 남고 7일·15일·30일·2인·3인·
                         * 5,000원이 「인원에 따라 서비스가 더 붙는다」로 뭉쳐졌다.
                         */
                        type === 'review'
                          ? '적어둔 금액·기간·인원 조건을 하나도 빼지 않고 방문객이 들은 말로 풀어 씁니다. 실제 조건만 적으세요 — 없는 마감일·인원은 만들지 않습니다.'
                          : '적어둔 조건을 하나도 빼지 않고 밝힙니다. 없는 마감일·인원을 만들어내지 않으니 실제 조건만 적으세요.'
                      }
                    >
                      <textarea
                        value={eventText}
                        ...
                        placeholder={
                          type === 'review'
                            ? '공동구매 3개월 99,000원 / 혼자 7일·2인 15일·3인 30일 서비스 / 네이버 5,000원 쿠폰'
                ...
                */}
                {ai?.label && (
                  <p className="muted mb-3 text-[11.5px] leading-relaxed">
                    ...
                    {type === 'info' &&
                      (ai.canSearch
                        ? ' — 기관·학회 자료를 찾아 「○○학회 자료에 따르면」처럼 출처를 문장 앞에 세워 인용합니다 (못 찾으면 인용 없이, 조건을 붙인 설명으로 씁니다).'
                        : ' — 이 키로는 자료를 찾을 수 없어 연구·수치 인용 없이 씁니다. 「휴대폰에서 쓰기 · 배포」 화면을 보세요.')}
                  </p>
                )}
...
function BodyCard({ pkg }: { pkg: ReturnType<typeof buildCopyPackage> }) {
  ...
  return (
    ...
    >
      ...
      {/*
        **줄바꿈이 안 넘어간다** (2026-08-31 회원 지적: "서식 복사해서 블로그에 붙여넣기
        하면 줄바꿈이 안된채로 와"). 무엇을 바꿨는지와, 저희가 확인할 수 없는 부분이
        어디까지인지 같이 적는다 — 「이제 됩니다」라고만 적으면 또 안 될 때 알 수가 없다.
      */}
      <p className="muted mt-2 text-[11px] leading-relaxed">
        <b>「서식 포함 복사」</b>로 붙이면 줄바꿈 · 소제목 위아래 <b>구분선</b> · <b>인용구</b>가 함께
        들어갑니다. 줄바꿈은 <b>줄마다 문단 하나</b>로 보냅니다 — 네이버 에디터에는 「문단 안 줄바꿈」이라는
        자리가 없어서, 문단 안에 넣은 줄바꿈은 붙여넣기에서 버려집니다. 빈 줄도 <b>빈 문단</b>으로 보냅니다
        (여백 값만 주면 스타일이 떨어질 때 간격이 통째로 사라집니다).
      </p>
      <p className="muted mt-1.5 text-[11px] leading-relaxed">
        네이버가 구분선·인용구를 자기 <b>컴포넌트</b>로 바꿔주는지는 에디터 버전마다 달라서, 선·굵기를 직접
        박아 보냅니다 — 컴포넌트로 안 바뀌어도 보이는 모양은 남습니다. 인용구의 <b>큰 따옴표 기호(“)</b>는
        네이버 자기 스타일(라인&amp;따옴표)에서만 나오니, 원하면 붙인 뒤 그 줄에서 <b>인용구 스타일만</b> 바꿔
        주세요. 그래도 모양이 이상하면 <b>「글자만」</b>으로 붙이고 서식은 네이버에서 넣는 편이 확실합니다.
      </p>
      <p className="muted mt-1.5 text-[11px] leading-relaxed">
        줄은 <b>글자수가 아니라 마디에서</b> 끊습니다 (쉼표 · ~고 · ~며 · ~한테 같은 자리). 그리고{' '}
        <b>함께 읽히는 말은 갈라놓지 않습니다</b> — 「정리해 보려고 합니다」·「할 수 있어서」·「~와 관련해서」처럼
        혼자서는 뜻이 서지 않는 말(보조용언 · 종결 서술어 · 의존명사)은 앞말에 붙여 한 묶음으로 보고, 줄은 그
        묶음 사이에서만 끊습니다. 길이도{' '}
        <b>글자수가 아니라 너비로</b> 잽니다 — 한글은 한 자가 네모 하나를 다 쓰고 공백 · 마침표 · 영문은 그
        절반도 안 되니, 글자수가 같아도 넘치는 줄과 안 넘치는 줄이 갈립니다 (회원님이 보내주신 화면에서
        29자는 넘쳤는데 28자는 들어갔습니다). 그래서 <b>한글 {LINE_MAX}자쯤</b>을 상한으로 두고, 마지막 줄에
        네다섯 글자만 남으면 앞 줄에서 한 낱말을 내려 줍니다. 두세 줄마다 빈 줄입니다. <b>가독성 판단이지 순위 규칙이 아닙니다</b> —
