...
export const TOPIC_SEEDS: { id: string; label: string; queries: string[] }[] = [
  ...
  /*
   * ─── 운동·식단 밖의 건강 정보 (2026-09-10 회원 요청) ────────────────────
   *
   * 회원: "주제를 꼭 운동과 식단에 한정하지 않아도 돼 그냥 건강 정보들도 다룰 수 있게
   * 주제를 업데이트 해줘."
   *
   * 여덟 갈래가 전부 운동·식단이었다. 아래 세 갈래는 몸에 붙어 있으면서 운동 밖인 것들이다.
   *
   * **병 이름을 씨앗에 넣지 않는다.** 「당뇨」·「고혈압」을 넣으면 자동완성이 치료·약 얘기를
   * 물어다 주고, 그 주제로 쓴 글은 검수의 위험 표현(C. 의료·치료성)에 걸린다. 대신
   * 「혈압 수치」·「인바디 보는 법」처럼 **읽고 관리하는 쪽**의 말만 넣는다.
   */
  {
    id: 'sleep',
    label: '잠 · 회복',
    queries: ['수면의 질', '잠 잘 오는 방법', '운동 후 회복', '근육통 풀기', '낮잠', '주말 몰아 자기', '자기 전 스트레칭', '수면 시간'],
  },
  {
    id: 'condition',
    label: '몸 상태 · 컨디션',
    queries: ['다리 부종', '만성 피로', '소화 잘 되는 법', '물 많이 마시기', '눈 피로', '스트레스 식욕', '손발 저림 스트레칭', '아침 붓기'],
  },
  {
    id: 'numbers',
    label: '수치 읽기 · 몸 관리',
    queries: ['체지방률', '기초대사량', '인바디 보는 법', 'BMI 계산', '하루 걸음 수', '체중 재는 시간', '허리둘레 기준', '혈압 수치'],
  },
]
...
const OFF_LIMIT_WORDS = [
  ...
  '탈출증',
  /*
   * ─── 2026-08-28, 회원이 실제 화면을 보고 짚었다 ────────────────
   *
   * "예신다이어트 / 칼로리바다이어트 … 그것도 걸러줘."
   *
   * 프로덕션 탐색 결과 48줄을 실제로 받아 보고 고른 것들이다. 갈래는 셋이다.
   *
   * ① **파는 물건** — 다이어트환(먹는 환약) · 탄수화물컷팅제 · 체지방컷팅제 · 단백질쿠키 ·
   *    칼로리바. 우리가 파는 것이 아니고, 효과를 말하면 건강기능식품 광고가 된다.
   * ② **남의 방법론 이름** — 클렌즈다이어트 · 연예인다이어트 · 원푸드다이어트 · 황제다이어트.
   *    남이 이름 붙인 방법이라 우리 글이 그 이름의 설명서가 된다.
   * ③ **우리가 다루면 위험한 몸 상태** — 임산부 · 산모 · 갱년기 · 혈당. 몸이 특수한
   *    상태이고 잘못 말하면 사람이 다친다. 병 이름은 이미 위에서 막고 있는데 이 셋은
   *    병 이름이 아니라 새어 나왔다.
   *
   * 「출산후다이어트」는 **넣지 않는다** — 산후 회복 운동은 헬스장이 실제로 하는 일이고,
   * 그걸 막으면 쓸 만한 주제를 잃는다. 막는 것은 임신 중(임산부·산모)이다.
   */
  '컷팅제',
  '쿠키',
  '젤리',
  ...
  '연예인',
  /*
   * 「예신다이어트」 — 「예신」은 결혼 준비 쪽 은어(예비신부)다. 회원이 콕 집어 말한 말이라
   * 넣는다. **다만 판단이다** — 헬스장이 예비신부를 대상으로 쓸 수도 있다. 「웨딩」·「결혼」은
   * 넣지 않았으니 그쪽 표현으로 올라오는 후보는 살아 있다.
   */
  '예신',
  '임산부',
  '산모',
...
/**
 * **숫자로 시작하는 동네** (2026-08-26). 「2동탄운동」이 그대로 올라왔다 — 앞에 숫자가
 * 붙으면 위 규칙의 `^[가-힣]` 에 안 걸린다.
 *
 * 「3대운동」·「5분운동」·「10분운동」은 걸리면 안 되므로, 숫자 뒤의 **동네 이름과 운동이
 * 모두** 있을 때만 잡는다 (「2 + 동탄 + 운동」). 「3 + 대운동」은 뒤에 붙는 말이 없어 통과한다.
 */
const NUMBERED_AREA_RE = /^\d+[가-힣]{2,4}(운동|헬스|짐|다이어트|필라테스|요가)/
/**
 * **접미사가 없는 지명** — 「잠실다이어트」·「광주다이어트」 (2026-08-28).
 *
 * `AREA_RE`·`AREA_ACTIVITY_RE` 는 「동·읍·면·구·시·군」이 붙은 말만 잡는다. 그런데 실제
 * 결과에 「잠실다이어트」와 「광주다이어트」가 그대로 올라왔다 — 회원이 이미 「매탄동운동」을
 * 두고 지적한 것과 **같은 갈래**인데, 접미사가 없어서 새어 나왔다.
 *
 * **이름 목록이라 끝이 없다.** 그래서 넓게 잡지 않고 **광역시·도 열일곱 개**와 헬스장
 * 검색에서 실제로 자주 붙는 상권 몇 개만 둔다. 우리 동네(쌍용동·천안)는 `myLocalKeywords`
 * 가 따로 잡으므로 여기 없어도 된다.
 *
 * 뒤에 우리 판 낱말이 **바로 붙을 때만** 잡는다 — 「경기력 향상」이 「경기」로 걸리면 안 된다.
 */
const CITY_RE =
  /^(서울|부산|대구|인천|광주|대전|울산|세종|경기|강원|충북|충남|전북|전남|경북|경남|제주|강남|잠실|홍대|신촌|건대|수원|성남|용인|고양|청주|전주|포항|창원)(다이어트|운동|헬스|짐|필라테스|요가|피티|pt)/i
/**
 * **숫자 뒤가 단위면 동네가 아니다.**
 *
 * 위 규칙만 두면 「60대 근력운동 순서」가 「60 + 대근력 + 운동」으로 걸린다 (실제로 걸렸다).
 * 「3개월운동」·「1시간운동」·「3대운동」도 같은 꼴이다 — 전부 멀쩡한 주제다.
 * 그래서 숫자 바로 뒤가 **세는 말**이면 지역 검사를 하지 않는다.
 */
const NUM_UNIT_RE = /^\d+(분|초|시간|시|주차|주|개월|달|일|년차|년|세트|세|대|회|번|kg|킬로|살|km)/i

/**
 * **「운동」이 몸 쓰는 일이 아닐 때** (2026-08-26 회원 지적: "아직도 주제가 이상하게 나와").
 *
 * 화면에 「동학농민운동 순서」가 올라왔다. 자동완성이 「운동 순서」를 물으면 역사 사건까지
 * 끌고 온다 — 우리가 쓸 주제가 아니고, 그걸로 글을 쓰면 블로그 주제도 흔들린다.
 *
 * **바로 앞에 붙은 말만** 본다. 「여성 근력운동」처럼 우리 말이 앞에 있는 것은 건드리지
 * 않으려고 목록을 역사·사회 용어로 좁혔다.
 */
const MOVEMENT_RE = /(동학|농민|독립|민주화|민주|학생|노동|의병|계몽|새마을|물산장려|국채보상|삼일|촛불|반대|서명)운동/

/**
 * 업체·장소·서비스를 가리키는 말.
 *
 * **관공서·시설을 넣었다** (2026-08-26). 「금천구청운동」이 주제로 올라왔다 — 「금천구청
 * 근처에서 운동할 곳」을 찾는 말이다. 「구청」·「시청」이 들어간 운동 주제는 없다.
 *
 * **커뮤니티·카페·앱도 넣었다.** 「헬스커뮤니티」는 정보를 찾는 말이 아니라 **모일 곳**을
 * 찾는 말이라 우리가 쓸 주제가 아니다.
 */
const PLACE_WORDS = [
  '헬스장',
  '피트니스',
...
export function classifyIntent(query: string, myLocalKeywords: string[] = []): TopicIntent {
  ...
  // 「매탄동운동」·「군자역운동」·「2동탄운동」 — 남의 동네에서 헬스장을 찾는 말이다 (2026-08-26)
  if (
    !DOMAIN_HEAD_RE.test(flat) &&
    (AREA_ACTIVITY_RE.test(flat) ||
      CITY_RE.test(flat) ||
      (!NUM_UNIT_RE.test(flat) && NUMBERED_AREA_RE.test(flat)))
  )
    return 'local'
  // 「동학농민운동」 — 여기서 「운동」은 몸 쓰는 일이 아니다 (2026-08-26)
  if (MOVEMENT_RE.test(flat)) return 'offlimit'
