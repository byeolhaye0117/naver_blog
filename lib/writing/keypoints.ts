 ...
 *
 * 「두 번째」·「첫 번째」는 **일상 명사구**다 — 「첫 번째에서」·「첫 번째 방식부터」처럼
 * 문장 속에서 앞을 가리키는 데 흔히 쓴다. 그래서 이 꼴은 **줄 맨 앞**을 요구하고,
 * 문장 끝 다음에 왔을 때는 아래 `SOFT_HEAD_TAIL` 로 한 번 더 가린다.
 *
 * (문장 **끝**에 단 「… 방식이 첫 번째입니다」는 또 다른 꼴이라 `TAIL_MARK` 가 따로 본다.)
 */
const SOFT_MARK = /((?:첫|한|두|세|네|다섯|여섯|일곱)\s*(?:번째(?:\s*단계)?|단계))/
/**
 * **문장 가운데의 「두 번째」가 항목인지 가리는 조사** (2026-08-30).
 *
 * 줄 맨 앞만 인정하면 놓치는 것이 있다 — 문단 중간에서 「… 나뉩니다. **첫 번째는** 복합
 * 운동입니다」처럼 항목을 여는 글이 있다. 그런데 그걸 살리려고 문장 끝 뒤를 전부 인정하면
 * 「덤벨 프레스입니다. **첫 번째에서** 쓰지 않은 각도를」이 다시 항목으로 올라온다
 * (회원이 본 그 화면이다).
 *
...
/**
 * **문장 끝에 단 순번** — 「… 고립 운동 나중 **방식이 첫 번째입니다.**」 (2026-08-30).
 *
 * 회원: "그래서 왜 안고치는거야?"
 *
 * 고쳐 쓰기가 왜 헛돌았는지 프로덕션에서 재보다 찾았다. 그 글의 두 번째 목록이 첫 항목을
 * **문장 끝**에 달아 뒀다 — 「크게 두 가지 방식으로 나뉘는데, 복합 운동 먼저, 고립 운동
 * 나중 방식이 첫 번째입니다.」 앞의 두 규칙(줄 맨 앞 · 문장 끝 다음의 주어 자리)으로는
 * 둘 다 안 걸려서 그 항목이 통째로 사라졌고, 남은 「두 번째는 사전 피로법」 하나만 보고
 * **「2번부터 시작합니다」**라고 짚었다.
 *
 * 그래서 고쳐 쓰기가 앞 목록을 제대로 고쳐도 이 지적이 남았고, 점수가 그대로라 라우트가
 * 「나아진 것이 없다」며 고친 글을 버렸다 (판정이 「수정필요가 같으면 점수가 높은 쪽」이다).
 * **없는 흠을 짚고 있었으니 고칠 방법이 없었던 것이다.**
 *
 * 서술어 자리(「…이 첫 번째입니다」)는 앞을 가리키는 말과 헷갈리지 않는다 — 문장이 거기서
 * 끝나기 때문이다. 「첫 번째 방식부터 시작하시고」처럼 뒤에 말이 이어지면 안 걸린다.
 */
const TAIL_MARK =
  /(첫|한|두|세|네|다섯|여섯|일곱)\s*(?:번째|단계)(?:\s*(?:방식|방법|단계))?\s*(?:입니다|이에요|예요|이다|이야|이고요)\s*[.!?]?\s*$/
...
export function keyPointsOf(body: string): KeyPoint[] {
  ...
  for (let li = 0; li < lines.length; li++) {
    ...
    /*
     * **문장 끝에 단 순번도 항목이다** (2026-08-30 — 위 TAIL_MARK 주석).
     *
     * 표시가 문장 맨 앞이 아니라 **끝**에 있으므로, 잡는 자리는 그 문장이 시작하는 곳이다
     * (문구가 「첫 번째입니다」 한 조각이 되면 안 된다). 이미 표시가 잡힌 문장은 건드리지
     * 않는다 — 같은 항목을 두 번 세게 된다.
     */
    let at = 0
    for (const sent of line.split(/(?<=[.!?])\s+/)) {
      const start = line.indexOf(sent, at)
      if (start < 0) continue
      at = start + sent.length
      const taken = marks.some((k) => k.at >= start && k.at < start + sent.length)
      const hit = TAIL_MARK.exec(sent.trim())
      if (!taken && hit) marks.push({ at: start, len: 0, n: markNumber(hit[1]) })
    }
