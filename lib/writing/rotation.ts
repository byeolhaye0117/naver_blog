 ...
 * 들어갔다(「교대근무자가 자주 묻는 질문 4가지」·「교대근무자도 지킬 수 있을까요?」·
 * 「교대근무자는 뭐부터 시작할까?」). 한 편은 20번 나왔다.
 *
 * **지시문이 그렇게 시켰다.** 정보글 1단계에 이렇게 적혀 있었다:
 *
 *   ①이 글을 누가 어떤 상황에서 읽는지 한 문장으로 못 박는다
 *     (예: "**교대근무라** 오후에 눈뜨고, 저녁 먹기 전에 한 시간 낼 수 있는 분")
 *
 * 인사 다음에 **바로 나오는 자리**인데 예시가 하나뿐이었다. 그리고 뒤쪽 신뢰 규칙에서
 * 「무릎이 아픈 분·**교대근무로 시간이 불규칙한 분**」으로 한 번 더 못을 박았다. 모델은
 * 예시를 그대로 베끼는 성질이 있어서, 매 글이 교대근무자 글이 됐다.
 *
 * 이 앱에서 이미 겪은 것과 같은 꼴이다 — 후기글 이벤트 구간의 「이런 혜택이 있었어요」도
 * 예시로 적어 둔 문장이 그대로 나왔다 (2026-09-02).
 *
 * **고치는 방식은 예시를 지우는 것이 아니라 돌리는 것이다.** 예시를 없애면 모델이 「많은
 * 분들이」로 돌아간다 — 그건 네이버가 나쁜 예로 든 것이고 검수도 막는다. 그래서 독자
 * 상황을 여러 개 두고, **최근 글에 안 나온 것**을 골라 준다 (도입 방식·앵글과 같은 방식).
 */
export const READER_PROFILES: { label: string; words: string[] }[] = [
  { label: '교대근무라 오후에 눈뜨고, 저녁 먹기 전에 한 시간 낼 수 있는 분', words: ['교대근무', '야간근무', '밤 근무'] },
  { label: '아이 등원시키고 오전에 한 시간 낼 수 있는 분', words: ['등원', '아이를 맡기', '유치원', '어린이집'] },
  { label: '앉아서 일하는 사무직이라 하루 걸음이 3천 보도 안 되는 분', words: ['사무직', '앉아서 일', '걸음', '보도 안'] },
  { label: '퇴근이 늦어 저녁을 9시 넘어 먹게 되는 분', words: ['퇴근이 늦', '9시 넘어', '늦은 저녁'] },
  { label: '주말에만 시간이 나서 이틀에 몰아 하려는 분', words: ['주말에만', '이틀에 몰아', '주 2회'] },
  { label: '헬스장은 처음이라 기구 앞에서 뭘 먼저 잡을지 모르는 분', words: ['처음이라', '기구 앞에서', '헬스장은 처음'] },
  { label: '다이어트를 세 번 해봤는데 매번 두 달쯤에서 멈춘 분', words: ['세 번 해봤', '두 달쯤', '매번 멈'] },
  { label: '학생이라 시험 기간에는 운동을 통째로 쉬게 되는 분', words: ['학생이라', '시험 기간', '방학'] },
]

/**
 * 이번 글의 독자 상황 — **최근 정보글에 안 나온 것**을 고른다 (순수 함수 — 테스트 대상).
 *
 * 최근 글을 훑어 이미 쓴 상황을 빼고, 남은 것 중에서 고른다. 전부 한 번씩 썼으면 가장
 * 오래된 것으로 돌아간다 (아무것도 못 고르면 예시가 사라져 「많은 분들이」로 돌아간다).
 */
export function pickReaderProfile(posts: Post[], seed = 0): string {
  const recent = posts
    .filter((p) => p.type === 'info')
    .sort((a, b) => (b.publishedAt ?? b.createdAt).localeCompare(a.publishedAt ?? a.createdAt))
    .slice(0, READER_PROFILES.length)
    .map((p) => `${p.title ?? ''}\n${p.body ?? ''}`)
    .join('\n')
  const used = (pf: (typeof READER_PROFILES)[number]) => pf.words.some((w) => recent.includes(w))
  const free = READER_PROFILES.filter((pf) => !used(pf))
  const from = free.length ? free : READER_PROFILES
  /*
   * 안 쓴 것이 여럿이면 그중에서도 돌린다 — 늘 목록 첫 줄을 집으면 「교대근무」가
   * 「등원」으로 바뀌기만 하고 또 매번 같은 상황이 된다.
   */
  const i = ((Math.trunc(seed) % from.length) + from.length) % from.length
  return from[i].label
}
...
/*
 * ─── 여덟 갈래로 넓혔다 (2026-09-10 회원 요청) ────────────────────────────
 *
 * 회원: "주제를 꼭 운동과 식단에 한정하지 않아도 돼 그냥 건강 정보들도 다룰 수 있게."
 *
 * 여섯 갈래는 전부 운동·식단 쪽이었다. 그리고 **하루 세 편을 쓰면 여섯 갈래가 이틀에 한
 * 바퀴 돈다** — 이틀 주기로 같은 주제군이 돌아온다는 뜻이다. 갈래를 넓히면 회원이 원한
 * 다양성과 유사문서 간격이 **같이** 나아진다 (여덟이면 사흘에 가까워진다).
 *
 * G·H 는 운동 밖이지만 몸에 붙어 있는 것만 담았다. **병·치료는 넣지 않는다** —
 * 그건 검수의 위험 표현(C. 의료·치료성)에 걸리는 자리다.
 */
/*
 * **기억하는 창은 후보 수보다 커야 한다** (2026-09-10, 검사가 잡았다).
 *
 * 축을 고를 때 최근 몇 편을 보는지가 이 값이다. 전에는 6편이었고 주제군도 6갈래였는데,
 * 주제군을 8갈래로 넓히자 **여덟 바퀴를 도는 사이에 앞의 것이 다시 「안 쓴 것」이 됐다** —
 * 여덟 갈래 중 일곱만 나오고 하나는 두 번 나왔다. 창이 짧으면 넓힌 만큼 못 돈다.
 *
 * 그래서 가장 큰 후보 목록보다 넉넉하게 잡는다. 여기 목록을 늘릴 때 이 값도 함께 본다.
 */
export const AXIS_WINDOW = 12
...
export const TOPIC_GROUPS = [
  ...
  'G. 몸 상태·컨디션 (부종·피로·소화·수분·눈 피로·스트레스)',
  'H. 수치 읽기 (체지방률·기초대사량·인바디·걸음 수 — 병이 아니라 읽고 관리하는 쪽)',
]
