...
export const SPECS: Record<PostType, Spec> = {
  ...
  info: {
    ...
    mainMax: 5,
    /*
     * ─── 목표를 하한에서 떼어 3 → 4 로 (2026-08-21) ────────────────
     *
     * 회원: "키워드 노출회수가 2번정도 밖에 안되는데 너무 적은거 아니야?"
     *
     * **순위 때문은 아니다.** 관찰소가 33번 잰 값이 있다 — 메인 키워드 횟수의 평균
     * advantage 는 **+0.046** 이고, 많이 쓴 쪽이 위였던 판 12회 · 적게 쓴 쪽이 위였던 판
     * 9회 · 차이 없음 12회다. 판마다 부호가 뒤집힌다 (+0.67 ~ -0.64). 같은 방식으로 잰
     * 최신성이 +0.361 인 것과 비교하면 사실상 잡음이다. **늘려도 안 오른다.**
     *
     * 그런데 **목표가 하한(3)에 붙어 있었다.** 통과 구간은 3~5인데 목표가 3이면, 모델이
     * 한 자리만 놓쳐도 2가 되어 즉시수정이다 — 회원이 본 「2번」이 그것이다. 목표를 4로
     * 옮기면 하나 놓쳐도 3이라 통과하고, 하나 더 써도 5라 통과한다. 여유를 가운데 둔다.
     *
     * 자리 목록도 따라 늘어난다 (지시문의 「자리 수가 곧 횟수다」) — 제목·인사·마무리에
     * 「방법」 구간이 붙어 4자리가 된다.
     */
    mainTarget: 4,
    densityMax: 1.8,
     ...
     *
     * **1 → 0** (2026-08-27). 회원: "그냥 정보성을 쓸때는 센타 입장에서 쓰는게 아니라
     * 일반 블로거가 쓰는 느낌으로 해줘."
     *
     * 위의 실측(상호명 유무로 순위 차이 없음)은 그대로 유효하다 — 이건 순위 판단이 아니라
     * **화자 판단**이다. 인사에 상호명이 있으면 그 한 줄로 화자가 업체가 된다. 뒤를 아무리
     * 깨끗하게 써도 첫 문장이 「저희 업체입니다」이면 나머지는 그 업체의 홍보로 읽힌다.
     * 지시문·골격·첫 문장도 같이 바꿨다 — 한쪽만 고치면 「골격은 시키는데 값이 없다」가
     * 된다 (08-21 에 겪었다).
     */
    legalNameMin: 0,
...
export function checkPost(input: CheckInput): CheckResult {
  ...
  if (input.type === 'info') {
    ...
    const marks: { label: string; re: RegExp; unless?: RegExp }[] = [
       ...
       */
      /*
       * **낱말 자체를 잡는다** (2026-08-21). 회원: "정보성에는 홍보문구가 아예 들어가면 안돼
       * 상담도 홍보문구잖아 알고 있는거지?"
       *
       * 맞는 지적이고, 우리 코드가 이미 그렇게 세고 있었다 — `CTA_WORDS = ['상담','예약','문의']`
       * 이고 홍보글은 그 횟수로 순위를 잰다. 그런데 정보글에서만 「제가 상담하면서 보면」을
       * 예외로 뒀었다. **같은 낱말을 한쪽에서는 홍보 신호로 세면서 다른 쪽에서는 아니라고
       * 하는 것**이라 앞뒤가 안 맞는다.
       *
       * 현장 근거를 대는 device 는 살아 있어야 하므로 지시문에 대체 표현을 줬다 —
       ...
       */
      {
        ...
        /*
         * **「등록」은 읽는 사람에게 시키는 꼴일 때만 잡는다** (2026-08-28).
         *
         ...
         *
         * 이건 **읽는 사람이 누구인지 적은 말**이지 우리에게 등록하라는 말이 아니다.
         * 게다가 골격이 그 문장을 시킨다 — 「①이 글을 누가 어떤 상황에서 읽는지 한 문장으로
         * 못 박는다」. **우리 지시문이 시키는 문장을 우리 검수가 막고 있었다.**
         *
         * 그래서 「등록」은 권유·거래 꼴에서만 잡는다 — 등록하세요·등록하시면·등록해 보세요·
         * 등록비·등록 문의·등록 혜택. 「등록하고」·「등록한 지」처럼 상황을 적은 말은 통과다.
         * 「칼로리」·「전문가와 상담」과 같은 갈래의 문제였다 (고칠 방법이 없는 즉시수정).
         */
        re: /([가-힣]{2,6}(?:와|과|에게|한테)\s*)?(상담|예약|문의|등록\s*(?:하세요|하시|해\s*보세요|해\s*주|하려면|비\b|비를|비는)|등록\s*(?:문의|혜택|이벤트|상담|기간|마감)|회원권|이용권)/,
       ...
       *
       * 앞선 판에서 「상담」을 막으면서 대체 표현으로 「회원분들 보면」을 권했었다. 그게
       * 잘못이었다 — 낱말만 바뀌었지 **「나는 이 사람들의 업체다」라는 관계는 그대로**다.
       * 읽는 사람에게 「당신도 등록하면 저 회원분들처럼 된다」로 읽히므로 홍보 문구가 맞다.
       *
       * 이제 근거를 댈 자리는 관계를 빼고 쓴다 — 「수업하다 보면」·「가르치다 보면」·
       * 「현장에서 보면」. 겪은 것을 말하되 **거래 관계를 말하지 않는다.**
       */
      { label: '회원·등록 관계', re: /회원\s*(님|분|들)|(신규|기존|저희|우리)\s*회원|등록\s*(하신|한|하는|하시는)\s*(회원|분|고객)|다니시는\s*(회원|분)/ },
      /*
       * **운영자·트레이너 신분이 드러나는 말** (2026-08-27 회원 요청: "그냥 정보성을
       * 쓸때는 센타 입장에서 쓰는게 아니라 일반 블로거가 쓰는 느낌으로 해줘").
       *
       * 08-27 오전에 「회원분들 보면」을 막으면서 대체 표현으로 「수업하다 보면」·
       * 「가르치다 보면」·「현장에서 보면」을 권했다. 그 다음 요청이 이것이다 — **그 말들도
       * 결국 「나는 여기서 가르치는 사람이다」를 말한다.** 화자가 업체면 글 전체가 그
       * 업체의 홍보로 읽히므로, 신분이 드러나는 자리를 통째로 막는다.
       *
       * 근거를 댈 자리는 없어지지 않는다 — 지시문이 출처(「대한비만학회 자료를 보면」)나
       * 조건을 붙인 설명(「하루 세 끼를 챙기기 어려운 경우라면」)으로 쓰게 한다.
       * **겪지 않은 개인 경험을 지어내는 쪽으로 열어주지는 않는다** — 이 글을 실제로 쓰는
       * 사람은 센터 운영자다.
       */
      {
        label: '운영자·트레이너 신분',
        re: /저희\s*(센터|지점|짐|헬스장|매장)|우리\s*(센터|지점|짐)|(수업|지도|코칭|피티|PT)\s*(하다|하면서|할\s*때|해\s*보면)|가르치(다\s*보면|면서|다가)|현장에서\s*보면|(제가|저희가)\s*(가르|지도|수업|코칭)|(트레이너|관장|센터장|운영자)로\s*(서|일)|오시면|방문해\s*주시|안내드립니다/,
      },
      {
        label: '방문·연락 유도',
     ...
     *
     * 08-21 에는 「인사말에 업체명 한번을 소개되게」 해서 1회를 허용했었다. 그때는 화자가
     * 센터였으니 자기 이름을 밝히는 것이 자연스러웠다. 화자가 일반 블로거가 된 지금은
     * 인사에 상호명이 있으면 **그 한 줄로 화자가 도로 업체가 된다.**
     */
    if (input.legalName && legalNameCount > 0) hit.push(`상호명 ${legalNameCount}회 (정보글에는 넣지 않습니다)`)

    add({
      id: 'info-purity',
      group: '내용 균형',
      label: '정보글 순수성 (업체를 드러내는 것)',
      // 걸리면 즉시수정 — 상호명도 포함이다 (2026-08-27, 위 주석 참고)
      level: hit.length ? 'fail' : 'pass',
      value: hit.length ? hit.join(' · ') : '없음 — 순수 정보글',
      target: '하나도 넣지 않는다',
      hint: hit.length
        ? '**정보글에서는 이 문장들을 빼세요.** 정보글의 목적은 블로그 지수를 쌓는 것이고, 전환은 홍보글이 맡습니다. 네이버 공식 공지가 「연락을 유도하는 활동」을 홍보성 게시물로 규정하므로, 위 요소가 들어가면 이 글이 정보글로 안 세어질 수 있습니다. 실측에서도 1페이지 정보형 38편 중 **25편(66%)이 이런 요소가 하나도 없었습니다**(전화번호는 5%뿐). 지우고 정보만 남기세요 — 전화·링크·위치·이벤트·「상담 주세요」 전부입니다. 정보글의 화자는 **업체가 아니라 일반 블로거**입니다 (2026-08-27 회원 결정). 「수업하다 보면」·「가르치다 보면」·「현장에서 보면」·「회원님들」·상호명은 전부 「나는 이 업체 사람이다」를 말하므로 뺍니다. 근거를 대던 자리는 **출처를 앞에 세우거나**(「대한비만학회 자료를 보면」) **조건을 붙인 설명**으로 바꾸세요(「하루 세 끼를 챙기기 어려운 경우라면」). 다만 겪지 않은 개인 경험을 지어내지는 마세요 — 「제가 3개월 해보니」는 이 글을 실제로 쓰는 사람이 겪은 일이 아닙니다.'
        : undefined,
      /*
   ...
   *   딱딱한 쪽: 1인칭(「제가」·「저는」)이 한 번도 없음 → 회사 공지문이 된다
   *
   * 2026-08-27: 정보글 화자가 일반 블로거가 되면서 「센터 1인칭」이라는 이름이 틀리게 됐다.
   * 재는 것은 같다 — **사람이 말하고 있는가**. 글쓴이가 업체든 블로거든 「제가」가 한 번도
   * 없으면 공지문으로 읽힌다. 정보글에서는 안내 문구만 바꾼다 (겪은 일을 지어내라는 말로
   * 읽히면 안 되므로 예시도 다르게 준다).
  ...
  add({
    ...
    value: loud
      ...
      : stiff
        ? `${input.type === 'info' ? '1인칭' : '센터 1인칭'} 0회`
        : '통과',
    target: '순위 기준이 아니라 우리 톤 기준입니다 (실측에서 톤은 순위와 무관했습니다)',
    hint: stiff
      ? input.type === 'info'
        ? '사람이 말하는 자리가 하나도 없어 공지문처럼 읽힙니다. 「제가」를 넣어 쓰는 사람이 있다는 걸 드러내세요 — 예) "제가 이 글에서 정리해 드릴 건 세 가지입니다." **겪은 일을 지어내라는 말이 아닙니다** — 없는 경험을 쓰지 말고 글을 이끄는 자리에만 쓰세요.'
   ...
   *
   * **정보글은 인사만 본다** (2026-08-27). 회원: "그냥 정보성을 쓸때는 센타 입장에서 쓰는게
   * 아니라 일반 블로거가 쓰는 느낌으로 해줘."
   *
   * 08-21 에는 정보글도 「인사 + 상호명」이었다 (회원 요청 "인사말에 업체명 한번을
   * 소개되게"). 화자가 센터였으니 자기 이름을 밝히는 것이 자연스러웠다. 화자가 일반
   * 블로거가 된 지금은 **그 한 줄로 화자가 도로 업체가 된다** — 뒤를 아무리 깨끗하게 써도
   * 첫 문장이 「저희 업체입니다」이면 나머지는 그 업체의 홍보로 읽힌다.
   *
   * 인사 자체는 남긴다 (08-21 회원 요청 "정보성글 인사말이 없어서 어색해"). 「안녕하세요.
   * 오늘은 …에 대해 정리해 보려고 합니다」로 열면 된다 — 지시문·골격·첫 문장도 그 꼴이다.
   *
   * 제목은 여전히 안 된다 (`info-title-purity`).
   */
  if (input.type !== 'review') {
    ...
    add({
      id: 'intro-greeting',
      group: '분량·구조',
      // 정보글은 상호명을 세지 않는다 — 화자가 업체가 아니다 (2026-08-27)
      label: input.type === 'info' ? '첫 문장 인사' : '첫 문장 인사 + 정식 상호명',
      level:
        input.type === 'info'
          ? greeted
            ? 'pass'
      ...
      target:
        input.type === 'info'
          ? '「안녕하세요. 오늘은 (메인 키워드)에 대해 정리해 보려고 합니다」로 시작 — 상호명은 넣지 않습니다'
          : '「안녕하세요, (정식 상호명)입니다」로 시작 (순위 기준이 아니라 우리 규칙)',
      hint:
        input.type === 'info'
          ? greeted
            ? undefined
            : '글 맨 처음을 「안녕하세요. 오늘은 (메인 키워드)에 대해 정리해 보려고 합니다」로 여세요. **상호명은 넣지 않습니다** — 이 글의 화자는 업체가 아니라 일반 블로거입니다.'
          : greeted && namedInOpening
            ? undefined
            : legal
  ...
  /*
   * ─── 후기글에 센터 말투가 섞였는지 (2026-09-01 회원 지적) ────────────
   *
   * 회원: "화자가 방문객인데 어떻게 제일 첫말에 상담오시는 분들이가 될 수 있어.
   * 이러면 화자가 방문객이 아니라 센타인거잖아."
   *
   * 실제로 나온 첫 문장이 이랬다:
   *
   *   **상담 오시는 분들이** 제일 먼저 물어보는 게 "저 운동 진짜 처음인데 여기 다녀도
   *   될까요"였습니다.
   *
   * 손님이 **우리에게** 온다는 시점이니 화자가 센터다. 그런데 검사는 통과였다 —
   * 목록에 「오시**면**」은 있는데 「오시**는**」이 없었다. **조사 하나 차이로 샜다.**
   *
   * 넓힐 때 조심한 것: 「오시는 분들」만으로 잡으면 「같이 오시는 분들도 많더라고요」처럼
   * **방문자가 다른 방문자를 가리키는 말**까지 걸린다. 그래서 **손님이 오는 목적**
   * (상담·문의·등록·견학·방문)이 앞에 붙었을 때만 잡는다 — 방문자는 그 자리에서
   * 「상담 갔더니」라고 쓰지 「상담 오시는」이라고 쓰지 않는다.
   */
  const OWNER_TONE = [
    // (아래 VISITOR_TITLE_WORDS 와 함께 화자 검사가 쓰는 목록이다)
    { p: /저희 (센터|지점|짐|헬스장)|우리 센터|우리 지점|저희 (트레이너|관장|직원|센터장)/, label: '소속 1인칭(「저희 센터」)' },
    { p: /오시면|방문해 주시|등록하시면|안내드립니다/, label: '센터가 손님을 부르는 말투' },
    /*
     * **손님이 우리에게 오는 시점** — 회원이 짚은 그 문장이 여기 걸린다.
     * 목적(상담·문의·등록·견학·방문)이 앞에 붙었을 때만 본다 (위 주석의 이유).
     */
    { p: /(상담|문의|등록|견학|방문|운동)\s*(오시는|오신|오세요|오시길|오시는지)/, label: '손님이 우리에게 오는 시점(「상담 오시는」)' },
    { p: /(문의|연락|찾아|이용해|와)\s*주시(면|는|고|어|세요)/, label: '센터가 손님에게 부탁하는 말투' },
    /*
     * **우리가 응대하는 시점.** 「수업하다 보면」·「상담하다 보면」은 정보글 순수성
     * 검사가 이미 같은 이유로 잡고 있는 말이다 — 후기글에서도 화자가 업체가 된다.
     */
    { p: /(수업|상담|가르치|운영|관리|응대)\S{0,3}\s*(하다\s*보면|하다\s*보니|해\s*보면)/, label: '현장에서 일하는 사람의 시점' },
    { p: /회원님/, label: '손님을 「회원님」이라 부르는 말투' },
  ]
  /*
