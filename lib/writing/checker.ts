...
export function checkPost(input: CheckInput): CheckResult {
  ...
  if (input.type === 'info') {
    ...
    add({
      ...
      target: '하나도 넣지 않는다',
      hint: promoHits
        ? '**정보글에서는 이 문장들을 빼세요.** 정보글의 목적은 블로그 지수를 쌓는 것이고, 전환은 홍보글이 맡습니다. 네이버 공식 공지가 「연락을 유도하는 활동」을 홍보성 게시물로 규정하므로, 위 요소가 들어가면 이 글이 정보글로 안 세어질 수 있습니다. 실측에서도 1페이지 정보형 38편 중 **25편(66%)이 이런 요소가 하나도 없었습니다**(전화번호는 5%뿐). 지우고 정보만 남기세요 — 전화·링크·위치·이벤트·「상담 주세요」 전부입니다. 「상담하면서 보면」·「등록하신 회원님들」처럼 근거를 대던 자리는 **거래 관계를 뺀 말**로 바꾸세요 — 「수업하다 보면」·「가르치다 보면」·「현장에서 보면」. 겪은 것은 말하되 「우리 회원」이라고 말하지 않습니다.'
        : hit.length
