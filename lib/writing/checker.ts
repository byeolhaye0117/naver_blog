...
export function checkPost(input: CheckInput): CheckResult {
   ...
   * ─── 확인할 수 없는 주장이 있는가 ──────────────────────────────
   *
   * 회원 요청 — "정보글은 팩트가 우선이야. 정확한 정보로 쓸 수 있게 해줘."
   *
   * 나온 글에 이런 문장이 있었다: 「식단 사진만 찍어놔도 충동적으로 먹는 횟수가 줄어든다는
   * **연구가 꽤 있습니다**」. 사실일 수도 있지만 **우리는 그 출처를 못 댄다.** 정보글은 이
   * 블로그의 신뢰를 쌓으려고 쓰는 글이라, 확인 못 하는 문장 하나가 그 목적을 정면으로 깎는다.
   *
   * **세 유형 다 본다.** 회원은 정보글 얘기로 말했지만, 없는 연구를 홍보글에 쓰면 그건 광고
   * 표시에 걸리는 자리다 — 「없는 리뷰를 만들지 않는다」(`review-honesty`)와 같은 갈래다.
   *
   * 두 가지만 잡는다. 좁게 잡는 대신 확실하게:
   *   ㉮ 출처 없는 연구·전문가 인용 — 「연구에 따르면」·「전문가들은」
   *   ㉯ 효과를 붙인 수치 — 「30% 줄어듭니다」 (「3개월 9.9만원」 같은 가격은 대상이 아니다)
   *
   * 본문에 링크나 「출처」가 있으면 **주의**로 낮춘다 — 근거를 댄 글까지 즉시수정으로 잡으면
   * 근거를 대는 일 자체를 막게 된다. 대신 사람이 한 번 보라고 적는다.
   */
  ...
  const hasSource = /https?:\/\/|출처/.test(input.body)
  if (citation || effectNum) {
    add({
      id: 'fact-source',
      group: '저품질 위험',
      label: '확인할 수 있는 주장인가',
      level: hasSource ? 'warn' : 'fail',
      value: [citation ? `출처 없는 인용: 「${citation}」` : '', effectNum ? `효과 수치: 「${effectNum}」` : '']
        .filter(Boolean)
        .join(' · '),
      target: '연구·전문가 인용 없음 · 효과를 숫자로 단정하지 않음',
      hint: hasSource
        ? '본문에 링크나 출처가 있어서 주의로만 잡았습니다. 그 문장이 정말 그 출처의 내용인지 한 번 확인해 주세요 — 아니면 우리 근거로 바꿔 쓰는 편이 안전합니다.'
        : `${
            citation
              ? '「연구에 따르면」·「전문가들은」은 출처를 댈 수 없으면 쓰지 않습니다. 같은 내용을 우리 근거로 바꾸세요 — 「제가 상담하면서 보면」·「회원분들 얘기를 들어보면」. '
              : ''
          }${
            effectNum
              ? '효과에 숫자를 붙이면 확인 못 하는 약속이 됩니다 — 「30% 줄어듭니다」 대신 「눈에 띄게 줄어드시더라고요」처럼 방향으로 쓰세요. '
              : ''
          }정보글은 이 블로그의 신뢰를 쌓으려고 쓰는 글이라, 확인 못 하는 문장 하나가 그 목적을 깎습니다.`,
      /*
