...
export function checkPost(input: CheckInput): CheckResult {
  ...
  /*
   * ─── 정보글에 전화번호가 박혔나 (2026-08-20) ──────────────────────
   *
   * 회원 질문: "정보성 블로그에 상호명이 들어가거나 홍보성이 되는(업체명·스마트플레이스
   * 링크·전화번호 등) 문구가 들어가?" 들어가고 있었다 — 지시문이 전화번호를 주고 있었다.
   *
   * 1페이지의 정보형 글이 실제로 어떻게 하는지 세어봤다 (지역 키워드 5개 × 1페이지 10편 중
   * 정보형 4편): **전화번호 0/4** (후기형 45% · 일상형 43%) · 가격·이벤트 0/4 · 예약 링크
   * 3/4 · 상담 권유 3/4. 정보형만 연락처가 0이다.
   *
   * 표본이 4편이라 `fail` 로 막지 않고 `warn` 으로 알린다. 상호명은 잡지 않는다 — 누가 쓴
   * 글인지 밝히는 것은 홍보가 아니고, 1페이지 정보글도 제목에서 정체를 밝힌다.
   */
  if (input.type === 'info') {
    const phone = /0\d{1,2}[-.\s]?\d{3,4}[-.\s]?\d{4}/.exec(bodyText)
    add({
      id: 'info-phone',
      group: '내용 균형',
      label: '정보글에 전화번호',
      level: phone ? 'warn' : 'pass',
      value: phone ? `있음 — ${phone[0]}` : '없음',
      target: '쓰지 않는다 (상담은 예약 링크로 연결)',
      hint: phone
        ? '정보글은 신뢰를 쌓으려고 쓰는 글이라, 연락처가 박히는 순간 홍보물로 읽힙니다. 실측에서 1페이지 정보형 글 4편 중 전화번호를 넣은 글은 **0편**이었고(후기형 45% · 일상형 43%), 링크·연결을 쓴 글은 3편이었습니다. 전화번호를 지우고 예약 링크로 바꾸세요. (표본이 4편이라 막지는 않습니다.)'
        : undefined,
      weight: 2,
    })
  }
