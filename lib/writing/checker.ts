...
export const SPECS: Record<PostType, Spec> = {
  ...
  info: {
    ...
    charMax: 3000,
    /*
     * **2 → 0** (2026-08-20). 회원이 영상을 주며 "정보성글 다시 개편하자"고 했다.
     * 영상(머니코치 최준호)이 든 여섯 가지 금지 항목 중 세 번째가 「과도한 상호명 반복」이고,
     * 근거는 네이버의 홍보성 게시물 공지다. 우리 실측에서도 정보형 38편 중 25편(66%)이 홍보
     * 요소가 하나도 없는 순수 정보글이었다.
     *
     * 앞 판의 2회는 회원 요청("화자는 센터로 해서 상호명도 함께 소개될 수 있게")에서 나온
     * 값이다. 그 요청을 이번 요청이 덮는다 — 조용히 바꾸지 않고 여기 적어 둔다.
     */
    legalNameMin: 0,
...
export function checkPost(input: CheckInput): CheckResult {
  ...
  if (input.type === 'info') {
    const marks: { label: string; re: RegExp }[] = [
      ...
      { label: '방문·연락 유도', re: /(연락\s*주세요|문의\s*주세요|상담\s*주세요|상담\s*받아\s*보세요|방문\s*주세요|방문해\s*보세요|들러\s*주세요|예약\s*하세요|신청\s*하세요|지금\s*방문)/ },
    ]
    const hit: string[] = []
    ...
    // 상호명은 따로 본다 — 설정에 상호명이 있을 때만 셀 수 있다
    if (input.legalName && legalNameCount > 0) hit.push(`상호명 ${legalNameCount}회`)
