...
export interface CopyPackage {
  ...
  /**
   * `#태그 #태그` 한 줄 — **본문 맨 마지막 줄에 붙이는 용도** (2026-08-26).
   *
   * 회원 요청: "지금 태그 붙일려면 하나씩 클릭해서 해야 하는데, 전체 복사해서 네이버
   * 블로그에 붙여넣으면 해시태그로 인식될 수 있게 바꿔줘."
   *
   * 태그 **칸**은 한 칸에 하나씩 넣는 곳이라 이 줄을 거기 붙이면 안 걸린다 (그건 그대로다).
   * 다만 네이버 본문에서 `#낱말`은 해시태그로 잡히므로, **본문 맨 아래에 이 한 줄**을 붙이면
   * 열한 번 누르지 않아도 된다. 붙인 뒤 파란 글씨로 바뀌었는지 눈으로 확인하는 절차를
   * 화면과 체크리스트에 함께 적어 둔다 — 에디터 버전에 따라 글자로만 남을 수 있고, 그러면
   * 예전처럼 태그 칸에 하나씩 넣으면 된다.
   */
  tags: string
...
export function buildCopyPackage(post: Post, store?: Store): CopyPackage {
  ...
  const checklist: { label: string; detail?: string }[] = [
     ...
     */
    {
      label: `해시태그 ${tagList.length}개 — 본문 맨 아래에 한 줄로 붙이기`,
      detail:
        '아래 태그 카드의 「#태그 한 줄 복사」를 본문 맨 마지막 줄에 붙이세요. 붙인 뒤 태그가 파란 글씨로 바뀌었는지 한 번 보시고, 글자로만 남았으면 태그 칸에 하나씩 붙이고 Enter 로 넣으세요 — 태그 칸은 한 칸에 하나씩 넣는 곳이라 한 줄을 통째로 붙이면 안 걸립니다',
    },
    /*
