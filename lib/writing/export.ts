...
export interface CopyPackage {
  ...
  /**
   * `#태그 #태그` 한 줄 — **보여주기용이다. 붙여넣기용이 아니다.**
   *
   * ── 두 번 시도했고 두 번 다 안 됐다 (2026-08-26) ─────────────
   * 회원 요청으로 「한 번에 전부」를 두 가지로 만들어 봤다.
   *   ① 태그 칸에 이 한 줄 → **태그 하나**로 들어갔다 (`#쌍용동헬스장,쌍용동헬스장PT…`)
   *   ② 본문 맨 아래에 이 한 줄 → 회원이 되돌리라고 했다 (태그는 태그 칸의 것이다)
   * 쉼표(`tagsPlain`)·줄바꿈도 마찬가지였다 — 회원 확인: "하나씩 붙이고 엔터를 해야
   * 태그로 인식이 되고 있어."
   *
   * 그래서 화면에는 **하나씩 순서대로 복사**만 둔다. 이 두 형태는 지우지 않고 남겨 두되
   * 버튼으로 내밀지 않는다 — 안 되는 버튼을 내밀면 그걸 먼저 눌러 보게 된다.
   */
  tags: string
  /** `#` 없이 쉼표로 — 위와 같은 이유로 화면에서는 쓰지 않는다 */
  tagsPlain: string
...
export function buildCopyPackage(post: Post, store?: Store): CopyPackage {
  ...
  const checklist: { label: string; detail?: string }[] = [
     ...
     */
    {
      label: `해시태그 ${tagList.length}개 — 하나씩 붙이고 Enter`,
      detail:
        '글 아래 「태그 편집」 칸을 한 번 누른 뒤, 아래 태그 카드의 버튼 → 붙여넣기 → Enter 를 반복하세요 (누를 때마다 다음 태그가 복사됩니다). 한 줄로 전부 붙이는 방법은 없습니다 — 쉼표로도 줄바꿈으로도 나뉘지 않고 태그 하나로 뭉쳐 들어갑니다. 태그 안에 공백이 있으면 거기서 끊기니 붙여 씁니다',
    },
    /*
