...
export function buildCopyPackage(post: Post, store?: Store): CopyPackage {
  ...
  const checklist: { label: string; detail?: string }[] = [
     ...
     */
    {
      label: `해시태그 ${tagList.length}개 — 글 아래 「태그 편집」 칸에`,
      detail:
        '아래 태그 카드의 「태그 칸에 한 번에 붙이기」를 붙여넣고 Enter — 개수만큼 나뉘면 끝입니다. 안 나뉘면 「줄바꿈으로」를 같은 방법으로, 그것도 안 되면 하나씩 복사해 붙이고 Enter 를 반복하세요. 태그 안에 공백이 있으면 거기서 끊기니 붙여 씁니다',
    },
    /*
