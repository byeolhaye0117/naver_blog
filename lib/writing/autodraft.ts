 ...
 * 주제를 업데이트 해줘."
 *
 * 열 개 다 운동·식단이었다. 헬스장 블로그를 보는 사람이 궁금해하는 것은 그보다 넓다 —
 * 잠·부종·소화·수분·자세·수치 읽는 법이 다 그 자리다. 아래 목록은 **몸 상태와 생활 습관**
 * 쪽을 함께 담았다.
 *
 * **다만 병 이야기는 넣지 않았다.** 「혈압 낮추는 법」·「당뇨 식단」처럼 병을 다루는 주제는
 * 곧바로 치료·효과 보장 표현으로 넘어가고, 그건 검수의 위험 표현(C. 의료·치료성)에
 * 걸리는 자리다. 대신 「혈압 수치 읽는 법」·「인바디 결과 보는 법」처럼 **읽고 관리하는
 * 쪽**으로 적었다. 지시문에도 같은 선을 그어 두었다 (lib/ai/prompt.ts).
 *
 * 판단 하나 밝혀 둔다: 주제가 너무 흩어지면 블로그의 전문성 신호가 흐려질 수 있다는 것은
 * **우리 판단이고 실측한 값이 아니다.** 그래서 몸·운동·생활 습관 안에서만 넓혔다.
 */
export const INFO_TOPICS: string[] = [
  ...
  '하루 걸음 수',
  /*
   * 세 낱말 안쪽·스무 자 안쪽으로 적는다 (`isTaggable`). 이 값이 **메인 키워드**가 되어
   * 제목 앞과 해시태그에 그대로 들어가므로, 문장이 아니라 **검색어** 꼴이어야 한다.
   */
  // 수치 읽기 (병이 아니라 읽고 관리하는 쪽)
  '체지방률 기준',
  '기초대사량 계산',
  '인바디 보는 법',
  'BMI 한계',
