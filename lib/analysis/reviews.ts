...
export interface PlaceReview {
  ...
  kind: 'text' | 'tag'
  /*
   * ─── 아래 셋은 「이게 진짜 리뷰다」를 보여주는 자리다 (2026-08-19 추가) ─────
   *
   * 회원 요청: "리뷰는 그대로 인용하면 되지, 글이랑 사진이랑 함께 첨부해서 홈페이지에 보일
   * 수 있게 해줘." 문장만 목록으로 두면 어느 리뷰에서 온 말인지 확인할 방법이 없다. 작성자·
   * 예약일·원본 화면을 함께 두면 **글에 인용한 문장을 원본과 눈으로 대조할 수 있다.**
   *
   * 전부 선택 항목이다 — 붙여넣기로 들어온 리뷰에는 없다.
   */
  /** 작성자 표시 이름 (네이버가 가리는 형태 그대로: ilo****) */
  author?: string
  /** 화면에 적힌 예약·방문 날짜 그대로 (「2.24」·「25.12.9」) */
  at?: string
  /** 원본 화면 이미지 경로 (앱 안 경로 또는 URL) — 글이 아니라 **근거**로 둔다 */
  image?: string
}
