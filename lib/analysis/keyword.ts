 ...
 *
 * 임계값은 실측 분포로 잡았다 (2026-08, 검색광고 API 실제 응답):
 *   봉명동헬스장 10 · 쌍용동헬스장 10 · 스쿼트자세 10 · 쌍용동PT 9 · 프로틴추천 8
 *   다이어트정체기 3 · 헬스초보 3
 * 지역+업종 키워드는 광고 슬롯이 거의 다 차고, 정보 키워드는 3 안쪽이다. 5 를
 * 경계로 두면 이 두 무리가 갈린다.
 *
 * 값이 곧 화면에 보이는 광고 개수는 아니다 — 모바일은 파워링크를 몇 개만 먼저
 * 보여주고 접는다. 그래서 「N개가 화면을 덮는다」고는 말하지 않고, 광고가 그만큼
 * 경합한다 = 상업성이 높다 + 그 블록이 블로그보다 위에 있다 까지만 말한다.
 */
