...
const STRUCTURE: Record<PostType, string> = {
  promo: [
     ...
     */
    '3) 해결 500~600자, 소제목 있음. 최대 비중. **시설 스펙을 나열하지 않는다.** 고민을 하나씩 해소하면서, 각각 "그래서 여기서 무엇을 어떻게 하면 되는지"까지 답한다. 예) "런닝머신 10대" (X) → "출근 전이면 유산소 15분부터, 경사 5도에 속도는 대화가 되는 정도. 숨이 차면 경사를 먼저 낮춥니다" (O). 자극이 어디에 오는지·호흡·반복·자세·동작 이름을 쓰고, 근력·부위·단백질 같은 원론적인 말로 채우지 않는다. 안 해본 운동·없는 기구·확인 안 된 수치는 쓰지 않는다.',
