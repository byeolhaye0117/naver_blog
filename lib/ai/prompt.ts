...
export function buildSystemPrompt(type: PostType): string {
  ...
  return [
     ...
     */
    `- 메인 키워드 ${spec.mainMin}~${spec.mainMax}회, 밀도 ${spec.densityMax}% 이내(스터핑 안전선). 하한(${spec.mainMin}회)은 지키되 **넘겼으면 더 넣지 않는다** — 실측에서 횟수와 순위의 관계가 키워드마다 정반대였고(1위가 17회인 판도, 0회인 판도 있었다) 더 넣어서 오르지는 않았다. 억지 문장을 만들지 말고 배치 슬롯 안에서 채운다.`,
    '- 함께 찾는 키워드는 각 1~2회. **없어도 된다** — 실측에서 상위 3위권 12편 중 6편이 0회였다. 자연스럽게 안 들어가면 넣지 않는다.',
