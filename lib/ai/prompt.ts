...
export function buildSystemPrompt(type: PostType): string {
  ...
  return [
    ...
    /*
     * 2026-08-06 실측으로 범위를 내렸다 (홍보글 5~7 → 3~6).
     * 상위 32편에서 횟수 상관이 키워드마다 정반대였고(+0.75 ~ -0.39), 1위가 17회인 판에
     * 1위가 0회인 판도 있었다. 함께 쓰는 키워드는 상위 3위권 절반이 0회였다.
     */
    `- 메인 키워드 ${spec.mainMin}~${spec.mainMax}회, 밀도 ${spec.densityMax}% 이내(스터핑 안전선). **많이 넣어서 오르는 게 아니다** — 실측에서 횟수와 순위의 관계가 키워드마다 정반대였고, 1위가 0회인 경우도 있었다. 하한만 맞추고 억지로 채우지 않는다.`,
