...
export interface AdKeywordRow {
  ...
  compIdx?: string
  /**
   * 이 키워드에 붙은 **파워링크** 광고 개수 (`plAvgDepth`. PL = 파워링크 = 사이트검색광고).
   *
   * 왜 보나. 이 검색어에 돈을 쓰는 업체가 몇 곳인지 = **상업성 세기**다. 처음에는
   * 「블로그가 아래로 밀리는 정도」로 썼는데 실측에서 근거가 무너졌다 — 우리 지역
   * 키워드의 모바일 통합검색에는 파워링크가 아예 없었다 (lib/analysis/keyword.ts 의
   * PLACE_ABOVE_BLOG 주석).
   */
  adDepth?: number
