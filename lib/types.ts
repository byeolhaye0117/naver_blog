...
export interface AutoDraftPlan {
  ...
  topics?: string[]
  /*
   * ── 날짜별 지정(days)은 없앴다 (2026-08-25) ──────────────────
   *
   * 회원: "내가 주제 계속 확정하는거 아니라 했잖아. 근데 왜 또 주제 고르라고 나오는거야."
   *
   * 「날짜별로 주제를 고르고 싶어」를 **날짜마다 회원이 주제를 확정한다**로 읽었던 것이
   * 잘못이다. 회원이 원한 것은 **날마다 다른 주제가 나오는 것**이고, 그건 이미 앱이
   * 로테이션으로 한다 (topics × keywords 를 오래 안 쓴 순서로 돈다).
   *
   * 날짜마다 확정하게 만들면 결국 손으로 쓰는 것과 같아진다 — 이 기능의 값이 사라진다.
   * 그래서 회원이 정하는 것은 **범위(keywords·topics)와 쉬는 날(skip)뿐**이고, 어느 날
   * 무엇을 쓸지는 앱이 정한다.
   */
  /**
