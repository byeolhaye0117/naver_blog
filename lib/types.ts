...
export interface AutoDraftPlan {
  ...
  topics?: string[]
  /**
   * **날짜별로 콕 집어 정한 것** (2026-08-24 회원 요청).
   *
   * "하나의 주제를 계속 쓰고 싶지 않아 날짜 설정해서 날짜별로 설정할 수 있게 해줘."
   *
   * 범위만 정하면(keywords·topics) 앱이 알아서 돌리는데, 고른 주제가 하나뿐이면 매일 같은
   * 주제가 나온다. **그 날 무엇을 쓸지 직접 정할 자리**가 필요하다.
   *
   * 여기 적힌 날은 로테이션보다 우선한다. 안 적은 날은 지금처럼 범위 안에서 돌아간다 —
   * 매일 하나씩 지정하게 만들지 않는 것이 이 기능의 값이다.
   *
   * ── 키워드는 안 받는다 (2026-08-25) ────────────────────────
   * 회원: "나는 모든 날짜를 하나의 주제로 통일하고 싶은게 아니라 날짜별로 주제를 고르고
   * 싶어." 그리고 "날짜고르기 누르면 다시 또 키워드랑 주제 정할 수 있게 되어 있는데?"
   *
   * 맞는 지적이다. 키워드는 ①에서 이미 고른다 — 날짜마다 또 고르게 하면 같은 것을 두 번
   * 묻는 셈이고, 정작 회원이 정하고 싶은 것은 **그 날 무슨 이야기를 쓰나**(주제)다.
   * 그래서 **주제만** 받고 키워드는 로테이션이 고른다.
   */
  days?: { date: string; topic: string; /** 예전에 저장된 값 — 있으면 그대로 쓴다 */ keyword?: string }[]
  /**
