...
export interface Post {
  ...
  promoNote?: string
  /**
   * 정보글에서 **인용할 근거·출처** (회원이 직접 적는다).
   *
   * 회원 지적 — "정보글인데 내용 자체는 팩트로 써야 하잖아. 그냥 감으로 이렇게 했더니
   * 이렇게 됬다더라 하면 안 되는 거잖아. 정확한 정보를 전달해야지. 실제 연구 결과 등이
   * 있으면 출처도 함께 인용해서 쓰는 게 좋을 거 같아."
   *
   * 맞는 말이고, 앞 판에서 내가 반대로 고쳤다 (「연구를 인용하지 않는다」). 문제는 인용이
   * 아니라 **출처 없는 인용**이었다. AI 는 자료를 찾아볼 수 없으니, 인용할 자료는 사람이
   * 여기 넣고 AI 는 **그 안에서만** 인용한다 (`fact-source` 가 출처 표기를 확인한다).
   */
  sourceNote?: string
  /**
   * 정보 구간에서 다룰 **주제 하나**.
