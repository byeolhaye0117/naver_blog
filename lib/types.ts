...
export interface AutoDraftPlan {
  ...
  perDay?: number
  /**
   * **정한 날에만 쓸까, 매일 쓸까** (2026-08-31 회원 지적).
   *
   * 회원: "근데 애초에 그날 하루 주제 설정한건데 멋대로 다음날 까지 간게 이상한거 아니야?"
   *
   * 맞는 말이다. 회원은 ③ 날짜별 주제에 **08-30 까지만** 채워 뒀는데 08-31 에도 세 편이
   * 나갔다. 그때까지 규칙은 「채워 둔 날은 그 주제로, 안 채운 날은 ① 목록에서 골라서
   * 매일」이었다 — 날짜를 정하는 행동을 「그 날만 쓴다」는 뜻으로 읽는 것이 자연스러운데
   * 우리는 「그 날 주제만 정한다」는 뜻으로 만들어 뒀다.
   *
   * 비워 두면 **날짜를 채워 둔 적이 있으면 정한 날에만, 채운 적이 없으면 매일**이다.
   * 날짜를 한 번도 안 쓰는 회원에게서 매일 초안을 뺏지 않으려고 그렇게 갈랐다.
   * 화면이 지금 어느 쪽인지 늘 적어 두고, 여기서 명시로 뒤집을 수 있다.
   */
  everyDay?: boolean
