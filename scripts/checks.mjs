...
/*
 * ─── 후기글 첫 문장이 센터 말투였다 (2026-09-01 회원 지적) ────────────
 *
 * 회원: "화자가 방문객인데 어떻게 제일 첫말에 상담오시는 분들이가 될 수 있어.
 * 이러면 화자가 방문객이 아니라 센타인거잖아."
 *
 * 실제로 나온 첫 문장이 이랬다 — 그리고 검수는 **99점·수정필요 0** 으로 통과시켰다:
 *
 *   **상담 오시는 분들이** 제일 먼저 물어보는 게 "저 운동 진짜 처음인데 여기 다녀도
 *   될까요"였습니다.
 *
 * 손님이 **우리에게** 온다는 시점이니 화자가 센터다. 목록에 「오시**면**」은 있는데
 * 「오시**는**」이 없어서 **조사 하나 차이로 샜다.**
 */
{
  const review = (body) =>
    checkPost({
      type: 'review',
      title: '쌍용동 헬스장 상담 받아본 후기',
      body,
      mainKeyword: '쌍용동 헬스장',
      subKeywords: [],
      tags: [],
  ...
  const caught = review('상담 오시는 분들이 제일 먼저 물어보는 게 "운동 처음인데 다녀도 될까요"였습니다.')
  ok(caught?.level === 'fail', '회원이 짚은 그 문장을 잡는다', `${caught?.level} · ${caught?.value}`)
  ok(caught?.value.includes('오시는'), '걸린 말을 그대로 보여준다', caught?.value)

  // 같은 갈래 — 우리가 응대하는 시점
  ok(review('회원님들이 자주 물어보시는 게 있었습니다.')?.level === 'fail', '「회원님」은 센터가 손님을 부르는 말이다')
  ok(review('상담하다 보면 다들 같은 걸 물어보십니다.')?.level === 'fail', '「상담하다 보면」은 일하는 사람의 시점')
  ok(review('문의 주시면 자세히 알려드립니다.')?.level === 'fail', '「문의 주시면」도 센터 말투')
  ok(review('저희 트레이너가 자세를 봐드립니다.')?.level === 'fail', '「저희 트레이너」도 소속 1인칭')

  /*
   * **헛짚으면 고칠 수 없는 항목이 된다.** 「오시는 분들」만으로 잡으면 방문자가 다른
   * 방문자를 가리키는 말까지 걸린다 — 그래서 손님이 오는 목적(상담·문의·등록)이 앞에
   * 붙었을 때만 본다. 아래 다섯 줄은 전부 방문객이 쓸 수 있는 말이라 통과해야 한다.
   */
  for (const line of [
    '상담 갔을 때 제가 제일 먼저 물어본 게 "운동 진짜 처음인데 다녀도 될까요"였습니다.',
    '같이 오시는 분들도 많더라고요. 저녁에는 두 명씩 오는 분들이 보였습니다.',
    '저희 가족이 같이 다닐 곳을 찾고 있었습니다. 우리 동네에서는 여기가 제일 가까웠어요.',
    '등록하고 나서 3주쯤 다녔는데 무릎이 편해졌습니다.',
    '트레이너분이 제 자세를 보시고 무게를 내려주셨어요.',
  ]) {
    const v = review(line)
    ok(v?.level === 'pass', `방문객 말투는 통과한다 — ${line.slice(0, 26)}`, `${v?.level} · ${v?.value}`)
  }

  /*
   * **지시문에도 그 자리를 적는다.** 지시문은 이미 「저희 센터」·「우리 지점」을 막고
   * 있었는데 모델이 쓴 것은 그 목록에 없는 꼴이었다 — 이 저장소는 지시문만으로 안 붙는
   * 것을 여러 번 겪었고, 그때마다 **실제로 나온 문장을 그대로 적어** 두는 쪽이 먹혔다.
   */
  {
    const { buildSystemPrompt: sysV } = require(`${OUT}/ai/prompt.js`)
    const rv = sysV('review')
