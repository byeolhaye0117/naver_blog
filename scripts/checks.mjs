...
ok(CTA_MIN_BY_TYPE.info === 0, '정보글은 0회 — 상담 유도 구간이 없어졌다')
{
  const infoClean = checkPost({
    ...
    localKeyword: '천안헬스장',
    tags: ['폭식멈추는방법'],
    body: '## 왜 저녁에 몰리나\n혈당이 낮게 유지되다 떨어지면서 생깁니다. 천안 헬스장에서 수업하다 보면 이 질문을 제일 많이 받습니다.',
  })
 ...
 */
ok(infoSys.includes('현장 근거를 댈 때 무심코 새므로'), '「상담」이 새는 자리를 콕 집어 막는다')
ok(infoSys.includes('「수업하다 보면」'), '대신 쓸 말을 준다 (현장 근거는 살린다)')
...
ok(infoSys.includes('거래 관계는 말하지 않는다'), '겪은 것은 말하되 거래 관계는 빼라고 한다')
...
// 인사 형식을 회원이 정했다 (2026-08-21)
ok(infoSys.includes('안녕하세요, (메인 키워드) (정식 상호명)입니다. 오늘은 (이 글의 주제)에 대해 소개해드리려고 합니다.'), '인사 형식을 그대로 준다')
ok(infoSys.includes('여기 1회까지다'), '상호명은 인사에서 1회만 쓰라고 한다')
 ...
 */
ok(infoSkeleton.includes('안녕하세요, 폭식 멈추는 방법 MTO 피트니스 쌍용점입니다'), '골격이 회원이 정한 인사 형식으로 연다 (2026-08-21)')
ok(infoSkeleton.includes('**⓪첫 문장은 인사다.**'), '골격도 인사로 열라고 적는다')
ok(infoSkeleton.includes('에 대해 소개해드리려고 합니다'), '골격도 회원이 정한 문형을 그대로 준다')
ok(infoSkeleton.includes('**여기 1회까지**'), '골격도 1회 제한을 적는다')
ok(infoSkeleton.includes('현장에서 들은 질문이다'), '골격도 화자를 센터로 적는다')
/*
 * 회원: "정보성에는 홍보문구가 아예 들어가면 안돼 상담도 홍보문구잖아."
 * 골격도 같은 말을 해야 한다 — 손으로 쓰는 판과 기계가 쓰는 판이 갈리면 사람 쪽이 틀린다.
 */
ok(infoSkeleton.includes('**「상담」·「예약」·「문의」는 쓰지 않는다**'), '골격도 홍보 낱말을 막는다')
...
const INFO_GOOD = `폭식 멈추는 방법을 찾으시는 분들이 많습니다. 제가 수업할 때 이 질문을 제일 많이 받아요.
 ...
 */
{
   ...
   */
  {
    ...
    /*
     * **현장 근거는 살아 있어야 한다.** 다 막아버리면 「팩트 우선」 규칙이 갈 곳을 잃고,
     * 모델이 출처를 지어내는 쪽으로 샌다 — 그게 더 나쁘다. 관계만 빼면 된다.
     */
    ok(rel('수업하다 보면 이 질문을 제일 많이 받습니다.').level === 'pass', '「수업하다 보면」은 통과한다 (거래 관계가 없다)')
    ok(rel('현장에서 보면 첫 달에 그만두는 경우가 많습니다.').level === 'pass', '「현장에서 보면」도 통과')
    ok(rel('가르치다 보면 자세가 먼저 무너집니다.').level === 'pass', '「가르치다 보면」도 통과')
    // 검수 안내가 대체 표현을 알려준다 — 「빼세요」만 하면 무엇으로 바꿀지 모른다
    ok(rel('회원님들이 물어보십니다.').hint.includes('가르치다 보면'), '무엇으로 바꾸면 되는지 말해준다')
  }
