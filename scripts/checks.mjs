 ...
 */
{
   ...
   */
  {
     ...
     */
    {
      ...
      /*
       * ─── 한 문장에 든 목록 · 목록이 둘일 때 (2026-09-01 회원 지적) ────────
       *
       * 회원이 발행 패키지 화면을 보내며 물었다: "이게 맞아?"
       *
       * 아니었다. 두 가지가 겹쳐 있었다.
       *
       *   ① 첫 번째는 체중 기준으로 단백질량을 정하는 방식이고, 두 번째는 체지방률을 …
       *   ① 첫 번째는 매일 같은 부위만 반복하는 겁니다.
       *   ② 두 번째는 무게를 늘리지 않고 …
       *   ③ 세 번째는 식단을 갑자기 줄이는 겁니다.
       *
       * ㉮ 첫 줄은 **항목이 둘**인데 하나로 뭉쳤다. 본문이 한 문장 안에 「첫 번째는 …이고,
       *    두 번째는 …입니다」로 썼는데, 우리는 문장이 끝난 다음만 봤으니 쉼표 뒤의
       *    「두 번째는」이 안 잡혔다.
       * ㉯ ① 이 두 번 떴다. 한 글에 목록이 둘인데(식단 두 방식 / 흔한 실수 셋) 화면이 한
       *    줄로 이어 그려서 번호가 되돌아간 고장처럼 보였다.
       */
      {
        const one = keyPointsOf('여기엔 두 가지가 있습니다. 첫 번째는 체중 기준이고, 두 번째는 체지방률 기준입니다.')
        ok(one.length === 2, '한 문장에 든 목록도 둘로 나눈다', JSON.stringify(one.map((k) => k.text)))
        ok(one[0].n === 1 && one[1].n === 2, '번호도 따로 읽는다', JSON.stringify(one.map((k) => k.n)))
        ok(one[0].group === one[1].group, '같은 문장이면 같은 묶음이다')
        // 쉼표 뒤에서도 **주어 조사가 붙었을 때만** 본다 — 앞을 가리키는 말은 여전히 아니다
        ok(
          keyPointsOf('각도를 바꿉니다, 첫 번째로 쓰지 않은 각도를 고르세요.').length === 0,
          '쉼표 뒤라도 「첫 번째로」는 항목이 아니다',
          JSON.stringify(keyPointsOf('각도를 바꿉니다, 첫 번째로 쓰지 않은 각도를 고르세요.'))
        )
        ok(
          keyPointsOf('두 가지 방식과, 두 번째 방식을 함께 씁니다.').length === 0,
          '쉼표 뒤의 「두 번째 방식을」도 항목이 아니다'
        )

        // 화면이 목록 사이에 선을 넣는다 — 붙여 그리면 1·2·1·2 로 읽힌다
        const ed3 = require('node:fs').readFileSync(new URL('../app/write/Editor.tsx', import.meta.url), 'utf8')
        ok(
          /pkg\.keyPoints\[i - 1\]\.group !== k\.group/.test(ed3),
          '묶음이 바뀌는 자리를 화면이 알아본다'
        )
        ok(ed3.includes('border-t pt-3'), '그 자리에 선을 넣는다')
        ok(ed3.includes('목록이 둘 이상이면 선으로 나눠'), '왜 선이 있는지 카드 설명에 적는다')
        // 복사한 글에서도 목록이 바뀌는 자리는 빈 줄로 띄운다
        ok(/const gap = i > 0 && pkg\.keyPoints\[i - 1\]\.group !== k\.group/.test(ed3), '복사 글도 묶음 사이를 띄운다')
      }
